...
function ContactsTabContent({
  ...
  const list = useMemo(() => {
    if (!supplierId) return [];
    // Filtra pelo supplierId para garantir que apenas contatos deste fornecedor apareçam
    return contacts.filter(
      (c) => c.supplierId === supplierId || String(c.supplierId) === supplierId
    );
  }, [contacts, supplierId]);
  ...
  const handleSubmit = useCallback(
    async (data: ContactFormData) => {
      ...
      try {
        ...
        const payload: IContact = {
          supplierId, // Vinculação com Supplier
    ...
    [supplierId, editingContact, createContact, updateContact, fetchContacts]
  );
  ...
  if (!supplierId) {
    return (
      <p className="text-muted-foreground text-sm italic">
        Salve o fornecedor antes de gerenciar contatos.
      </p>
    );
  }
