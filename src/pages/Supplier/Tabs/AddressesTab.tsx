...
import type { IState, ICity } from "@/interfaces/locations.interface";
...
type AddressesTabProps = {
  supplierId: string | null; // Alterado de clientId
  disabled?: boolean;
};
...
function AddressesTabContent({
  ...
  useEffect(() => {
    ...
    fetchAddresses();
  }, [supplierId, fetchAddresses]);

  const list = useMemo(() => {
    if (!supplierId) return [];
    // Filtra pelo supplierId em vez de clientId
    return addresses.filter(
      (a) => a.supplierId === supplierId || String(a.supplierId) === supplierId
    );
  }, [addresses, supplierId]);
  ...
  const handleSubmit = useCallback(
    async (data: AddressFormData) => {
      ...
      try {
        ...
        const payload: IAddress = {
          supplierId, // Vincula ao fornecedor
    ...
    [supplierId, editingAddress, createAddress, updateAddress, fetchAddresses]
  );
  ...
  if (!supplierId) {
    return (
      <p className="text-muted-foreground text-sm">
        Salve o fornecedor antes de gerenciar endereços.
      </p>
    );
  }
